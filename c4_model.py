import numpy as np
import tensorflow as tf  # Tensorflow for the Deep Q Network
...
class Ramp(object):  # decreases epsilon like a "ramp"
    ...
    def step(self, steps: int) -> float:
        ...
        b = self.start
        y = m * x + b  # linearly decreasing epsilon
...
class C4Model(object):
    ...
    def train(self, experience: C4Experience):
        ...
        history = self._model.fit(
            experience.old_state.state_representation(), target_f, epochs=1, verbose=0
        )
    ...
    def print_suggest(self, state: C4State):  # prints better moves at the top and so on
    ...
    def predict(
        self, state: C4State, valid_moves: np.ndarray, epsilon: float = None
    ) -> C4Action:
        ...
        np.place(valid_moves, valid_moves == 0.0, [-999999.0])
        np.place(valid_moves, valid_moves == 1.0, 0.0)
