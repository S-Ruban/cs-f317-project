...
from enum import Enum, unique  # for enumerating constants
...
class C4State(object):
    ...
    def invert_perspective(self):
        ...
        np.place(
            self.state, self.state == C4SlotState.ENEMY.value, [C4SlotState.SELF.value]
        )
    ...
    def move_values(self) -> np.ndarray:
        ...
        for col_index in range(0, 7):
            ...
            ss = [
                min(3.0, ss_down),
                min(3.0, ss_right + ss_left),
                min(3.0, ss_up_right + ss_down_left),
                min(3.0, ss_down_right + ss_up_left),
            ]
            ee = [
                min(3.0, ee_down),
                min(3.0, ee_right + ee_left),
                min(3.0, ee_up_right + ee_down_left),
                min(3.0, ee_down_right + ee_up_left),
            ]

            s = [
                min(3.0, s_down),
                min(3.0, s_right + s_left),
                min(3.0, s_up_right + s_down_left),
                min(3.0, s_down_right + s_up_left),
            ]
            e = [
                min(3.0, e_down),
                min(3.0, e_right + e_left),
                min(3.0, e_up_right + e_down_left),
                min(3.0, e_down_right + e_up_left),
            ]
    ...
    def state_representation(self):
        return [
            self.one_hot(),
            self.move_values(),
            np.array([self.column_height()]) / 5.0,
        ]
...
class C4Game(object):
    def __init__(self):
        ...
        self._red_last_score = 0.0
        self._black_last_score = 0.0
    ...
    def reset(self):
        ...
        if (
            self.first_turn == C4Team.RED
        ):  # red and black alternate each game starting fiest
    ...
    def action(self, action: C4Action) -> C4MoveResult:
        ...
        if not self.duplicate:
            if self.current_turn() == C4Team.RED:
                ...
                if move_result == C4MoveResult.VICTORY:
                    self.black_memories[-1].reward = -1.0
                ...
                if move_result == C4MoveResult.VICTORY:
                    self.red_memories[-1].reward = -1.0
    ...
    def best_action(self, valid_moves: list, epsilon: float = 0.0):
