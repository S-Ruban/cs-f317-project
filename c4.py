...
import csv  # to dump training data into a .csv file
import os
...
from c4_model import C4Model  # importing the agent


def human_vs_human(weights_file: str):
    c4 = C4Game()  # create new game
    c4ai = C4Model(epsilon_start=0.0, epsilon_end=0.0)
...
def human_vs_ai(weights_file: str):
    ...
    )  # epsilon = 0 so that agent exploits training data to perform the best possible move every time
    c4ai.load(weights_file)
    ...
    while True:
        ...
        if current_team == C4Team.BLACK:
            ...
            move = c4ai.predict(
                c4.state, valid_moves=valid_moves
    ...
    while True:
        ...
        if best_wins + ai_wins >= games:
            return (
                best_wins,
                ai_wins,
                np.sum(best_scores) / games,
                np.sum(ai_scores) / games,
            )
    ...
    while True:
        ...
        if result == C4MoveResult.VICTORY:
            ...
            if training_data is not None:
                ...
                stats["game_length"] = (c4.turn + 1) / 42.0
                ...
                print(
                    "Epsilon: %f Gamma: %f Loss: %f mae: %f"
                    % (
                        c4ai.epsilon.value,
                        c4ai.gamma,
                        loss_sum / step_count,
                ...
                if (
                    game_no != 0 and game_no % 100 == 0
                ) or c4ai.steps >= training_steps:
                    print("Saving...")
...
if __name__ == "__main__":
    ...
    # parses CLA to train agent with different hyperparameters
